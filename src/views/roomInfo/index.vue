<template>
  <el-card class="box-card" shadow="always">
    <div slot="header">
      <span>房间信息管理</span>
      <el-button type="primary" style="float: right; padding: 6px 12px" ><input type="number" v-model="roomNumber" style="border: 0;outline: none" placeholder="根据房间号查询">&nbsp;&nbsp;&nbsp;<span @click="getRoomInfoByRoomNumber">搜索</span></el-button>
    </div>
    <el-table
      ref="multipleTable"
      :data="list"
...
import { getAllRoom, delRoom,batchDelRoomInfo,getByRoomNumber} from "../../api/roomInfo";
import {getByPhone} from "../../api/order";
...
export default {
  ...
  methods: {
    getRoomInfoByRoomNumber(){
      if(this.roomNumber==''){
        this.fetchData()
        return
      }
      getByRoomNumber(this.roomNumber).then((res)=>{
        if(res.code===1000) {
          this.list = res.data;
    ...
    batchDeletion: function () {
      ...
      console.log(array)
      batchDelRoomInfo(array).then((res)=>{
        if (res.code === 1000) {
          this.$message({
            message: '删除成功！',
            type: 'success'
